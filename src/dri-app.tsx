...
import { Gauge, AlertTriangle, Layers, FileBarChart, Settings as Cog, ChevronDown, ChevronRight, Plus, Trash2, Download, Calendar, GitBranch, List, Upload, FileDown, CheckSquare, Square } from "lucide-react";
...
// blocks carry weight; bricks are tasks inside a block
const seedBlocks = [
    { id: "bl1", name: "Data migration", weight: 30 },
    { id: "bl2", name: "People & training", weight: 25 },
    { id: "bl3", name: "Legal & compliance", weight: 20 },
    { id: "bl4", name: "Infrastructure", weight: 15 },
    { id: "bl5", name: "Stakeholder alignment", weight: 10 },
];
const seedBricks = [
...
export default function App() {
    ...
    const readiness = (cid, wid) => {
        const totW = blocks.reduce((a, b) => a + b.weight, 0) || 1;
        return Math.round(blocks.reduce((a, b) => a + blockScore(cid, wid, b.id) * b.weight, 0) / totW);
    };
    ...
    const renderM1 = () => (
        <>
            <div className="mb-4 flex flex-wrap items-end gap-3">
                ...
                <p className="text-sm" style={{ color: C.soft }}>Readiness = weighted average of <b>block</b> scores. A block's score = % of its <b>bricks</b> (tasks) completed.</p>
            </div>
    ...
    const renderM3 = () => (
        <>
            <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                <p className="text-sm" style={{ color: C.soft }}>Blocks carry the <b>weight</b> and appear in the readiness score. Bricks are the <b>tasks</b> inside each block. Total weight: <b style={{ color: totW === 100 ? C.low : C.med }}>{totW}</b>{totW !== 100 ? " (aim for 100)" : ""}.</p>
                <Btn onClick={() => setBlocks([...blocks, { id: gid(), name: "New block", weight: 0 }])}><Plus size={16} /> Add block</Btn>
            </div>
            <div className="grid gap-2">
                {blocks.map((bl) => {
    ...
    const Stat = ({ label, value, bg }) => (<Card bg={bg}><div className="text-3xl font-bold" style={{ color: C.ink }}>{value}</div><div className="text-sm" style={{ color: C.soft }}>{label}</div></Card>);
    ...
    const downloadTemplate = () => {
        ...
        add("BusinessUnits", [{ name: "Retail" }]); add("Blocks", [{ name: "Data migration", weight: 30 }]);
    ...
    const onImport = (e) => {
        ...
        reader.onload = (ev) => {
            try {
                ...
                const Bl = sh("Blocks").map((r) => ({ id: str(r.id) || gid(), name: str(r.name), weight: Number(r.weight) || 0 })).filter((b) => b.name);
                ...
                const cBy = {}; Cn.forEach((c) => cBy[c.name.toLowerCase()] = c.id); const wBy = {}; W.forEach((w) => wBy[w.name.toLowerCase()] = w.id);
    ...
    const renderM5 = () => (
        <>
            <Card bg={C.yellow} style={{ marginBottom: 16 }}>
                ...
                <p className="mb-3 text-sm" style={{ color: C.soft }}>Upload an .xlsx with sheets named Regions, Countries, Waves, Offers, BusinessUnits, Blocks, Bricks, Obstacles. Importing replaces matching data.</p>
                <div className="flex flex-wrap items-center gap-2">
    ...
    return (
        <div className="min-h-screen w-full" style={{ background: "#f4f7fc", color: C.ink, fontFamily: "system-ui, sans-serif" }}>
            <div className="mx-auto flex max-w-6xl flex-col sm:flex-row">
                <nav aria-label="Modules" className="shrink-0 p-3 sm:w-60" style={{ background: C.mid }}>
                ...
                <main className="flex-1 p-4 sm:p-6">
                    <header className="mb-4 flex items-center gap-2">{active && <active.Icon size={22} aria-hidden style={{ color: C.mid }} />}<h1 className="text-xl font-bold" style={{ color: C.ink }}>{active?.name}</h1></header>
                    {body()}
                </main>
            </div>
        </div>
